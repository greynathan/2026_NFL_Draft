#!/usr/bin/env python3
"""
Scrape MockDraftable for arm length only for players in our wr_training.csv
and wr_testing.csv. Saves to data/raw/mockdraftable_wr_arm_length.csv
for use in WR data_cleaning (merge by Player + Year).
"""
import re
import time
import csv
...
# Paths: script in WR/, data in project root
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)
DATA_DIR = os.path.join(PROJECT_ROOT, "data")
TRAINING_PATH = os.path.join(DATA_DIR, "processed", "wr_training.csv")
TESTING_PATH = os.path.join(DATA_DIR, "processed", "wr_testing.csv")
DRAFTED_2026_PATH = os.path.join(SCRIPT_DIR, "wr_drafted_2026.csv")
OUTPUT_PATH = os.path.join(DATA_DIR, "raw", "mockdraftable_wr_arm_length.csv")
...
def main():
    ...
    print(f"Loaded {len(players)} unique players from wr_training + wr_testing.")
