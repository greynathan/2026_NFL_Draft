"""
TE (Tight End) data cleaning: combine TEs + PFF Receiving + PFF Run_Blocking + RAS + arm length.

- Training 2015–2023 from nfl_combine_2010_to_2023.csv (Pos == 'TE').
- Testing 2024 from data/raw/2024 Draft - Public - TE.csv.
- Testing 2025 from data/raw/GabrielGTB 2025 NFL Combine - Master List.csv (Position == 'TE')
  + 2025_draft_picks.csv for Round/Pick.
- Testing 2026 from data/raw/2026_Combine/NFL Combine 2026 @JordanSportGuy Twitter - TEs.csv.
- PFF Receiving: from data/raw/pff/Receiving/*_receiving_summary.csv (position == 'TE').
- PFF Run Blocking: from data/raw/pff/Run_Blocking/*_offense_run_blockng.csv (position == 'TE').
- RAS: from data/raw/ras.csv.
- Arm length: from data/raw/mockdraftable_te_arm_length.csv if available.

Outputs (written to data/processed/):
- te_training.csv  (2015–2023)
- te_testing.csv   (2024–2026)

Run from project root:
    python TE/data_cleaning.py
"""
...
def normalize_pff_school(name):
    ...
    mapping = {
        ...
        "KANSAS ST": "Kansas State", "S DIEGO ST": "San Diego State",
        "W MICHIGAN": "Western Michigan", "W VIRGINIA": "West Virginia",
        "TEXAS A&M": "Texas A&M", "NWESTERN": "Northwestern",
        ...
        "C MICHIGAN": "Central Michigan", "DOMINION": "Old Dominion",
        "TCU": "TCU", "UNLV": "UNLV", "FLORIDA": "Florida",
        ...
        "UTAH ST": "Utah State", "WYOMING": "Wyoming",
        "HAWAII": "Hawaii", "FRESNO ST": "Fresno State", "NEVADA": "Nevada",
        "FIU": "Florida International", "GA SOUTHRN": "Georgia Southern",
        "GEORGIA SOUTHERN": "Georgia Southern", "ARK STATE": "Arkansas State",
...
def normalize_combine_school(name):
    ...
    upper_mapping = {
        ...
        "NC STATE": "North Carolina State", "NORTH CAROLINA": "North Carolina",
        ...
        "LIBERTY": "Liberty", "BOWLING GREEN": "Bowling Green",
    ...
    alias = {
        ...
        "Georgia Tech": "Georgia Tech", "Louisiana State": "LSU",
        "KANSAS ST": "Kansas State", "Kansas St.": "Kansas State",
    }
    return alias.get(x, x)


# ─── Load PFF Receiving data (TE only) ────────────────────────────────────────

PFF_RECEIVING_DIR = os.path.join(DATA_RAW, "pff", "Receiving")
TE_PFF_RECV_POSITIONS = {"TE", "Y"}
...
for year in range(2014, 2026):
    ...
    if "position" in sub.columns:
        sub = sub[sub["position"].astype(str).str.strip().str.upper().isin(TE_PFF_RECV_POSITIONS)].copy()
    sub["Year"] = year
    sub = sub.rename(columns={"player": "Player", "team_name": "School"})
    for c in receiving_cols[3:]:
        if c in sub.columns:
            sub[c] = pd.to_numeric(sub[c], errors="coerce")
    receiving_files.append(sub.drop(columns=["position"], errors="ignore"))
    print(f"Loaded PFF receiving (TE) {year}: {len(sub)} players")
...
if receiving_files:
    ...
    print(f"PFF TE receiving records (deduped): {len(receiving_data)}")
else:
...
# ─── Load PFF Run Blocking data (TE only) ─────────────────────────────────────

PFF_RUN_BLOCKING_DIR = os.path.join(DATA_RAW, "pff", "Run_Blocking")
TE_PFF_BLOCK_POSITIONS = {"TE", "Y"}
...
for year in range(2014, 2026):
    ...
    if not os.path.exists(path):
        continue
    df_rb = pd.read_csv(path)
    sub = df_rb[[c for c in run_block_cols if c in df_rb.columns]].copy()
    if "player" not in sub.columns or "snap_counts_run_block" not in sub.columns:
        continue
    if "position" in sub.columns:
        sub = sub[sub["position"].astype(str).str.strip().str.upper().isin(TE_PFF_BLOCK_POSITIONS)].copy()
    sub["Year"] = year
    sub = sub.rename(columns={"player": "Player", "team_name": "School"})
    for c in ["grades_run_block", "snap_counts_run_block"]:
        if c in sub.columns:
            sub[c] = pd.to_numeric(sub[c], errors="coerce")
    run_block_files.append(sub.drop(columns=["position"], errors="ignore"))
    print(f"Loaded PFF run blocking (TE) {year}: {len(sub)} players")

if run_block_files:
    run_block_data = pd.concat(run_block_files, ignore_index=True)
    run_block_data = (
        run_block_data.sort_values("snap_counts_run_block", ascending=False)
        .drop_duplicates(subset=["Player", "School", "Year"], keep="first")
        .reset_index(drop=True)
    )
    print(f"PFF TE run blocking records (deduped): {len(run_block_data)}")
else:
    run_block_data = pd.DataFrame(
        columns=["Player", "School", "Year", "grades_run_block", "snap_counts_run_block"]
    )
    print("No PFF run blocking files found for TE.")
...
def add_pff_receiving(combine_df: pd.DataFrame, pff_df: pd.DataFrame) -> pd.DataFrame:
    if pff_df.empty:
        for c in ["player_game_count", "yprr", "yards_per_reception", "caught_percent",
    ...
    def lookup(row):
        ...
        school = row["School_n"]
        mask = (pff_n["Player_n"] == player) & (pff_n["School_n"] == school) & (pff_n["Year"] == season)
...
def add_pff_run_blocking(combine_df: pd.DataFrame, rb_df: pd.DataFrame) -> pd.DataFrame:
    if rb_df.empty:
        combine_df["grades_run_block"] = np.nan
        return combine_df

    combine_df = combine_df.copy()
    rb_n = rb_df.copy()
    rb_n["School_n"] = rb_n["School"].apply(normalize_pff_school)
    rb_n["Player_n"] = rb_n["Player"].apply(normalize_player_name)
    combine_df["School_n"] = combine_df["School"].apply(normalize_combine_school)
    value_cols = [c for c in rb_df.columns if c not in ("Player", "School", "Year")]

    def lookup(row):
        season = int(row["Year"]) - 1
        player = normalize_player_name(row["Player"])
        school = row["School_n"]
        mask = (rb_n["Player_n"] == player) & (rb_n["School_n"] == school) & (rb_n["Year"] == season)
        match = rb_n.loc[mask]
        if match.empty:
            mask2 = (rb_n["Player_n"] == player) & (rb_n["Year"] == season)
            match = rb_n.loc[mask2]
        if match.empty:
            return pd.Series({c: np.nan for c in value_cols})
        return pd.Series({c: match.iloc[0][c] for c in value_cols})
...
def add_ras_data(combine_df: pd.DataFrame, ras_path: str) -> pd.DataFrame:
    ...
    ras_school = {
        ...
        "Virginia Tech": "Virginia Tech", "Colorado State": "Colorado State",
...
training_raw = nfl_combine_data[
    (nfl_combine_data["Pos"].isin(TE_POSITIONS)) & (nfl_combine_data["Year"].between(2015, 2023))
].copy()
...
print(f"TE training rows (2015–2023): {len(training_raw)}")


# ─── Load 2024 TEs from public draft file ────────────────────────────────────
# Note: 2024 TE file uses '3Cone' (not '3c') and 'SS' (not 'Shuttle') and 'Bench' (not 'BP')

draft_2024_path = os.path.join(DATA_RAW, "2024 Draft - Public - TE.csv")
te_2024_list = []
if os.path.exists(draft_2024_path):
    ...
    d24 = d24[d24["Pos"].astype(str).str.strip().str.upper() == "TE"].copy()
    for _, row in d24.iterrows():
        ...
        try:
            ...
            pass
        te_2024_list.append({
            "Year": 2024, "Player": str(row["Name"]).strip(), "Pos": "TE",
    ...
    print(f"Loaded 2024 TEs: {len(te_2024_list)}")
else:
    print("2024 Draft - Public - TE.csv not found.")

te_2024 = pd.DataFrame(te_2024_list)


# ─── Load 2025 TEs from GabrielGTB + draft picks ─────────────────────────────
...
if os.path.exists(combine_2025_path):
    ...
    c25_te = c25[c25["Position"].astype(str).str.strip().str.upper() == "TE"].copy()
    for _, row in c25_te.iterrows():
        te_2025_list.append({
            "Year": 2025, "Player": str(row["Name"]).strip(), "Pos": "TE",
    ...
    print(f"Loaded 2025 TEs from GabrielGTB: {len(te_2025_list)}")
else:
    print("GabrielGTB 2025 NFL Combine - Master List.csv not found.")

te_2025 = pd.DataFrame(te_2025_list)

if not te_2025.empty and os.path.exists(draft_picks_2025_path):
    dp25 = pd.read_csv(draft_picks_2025_path)
    dp_te = dp25[dp25["Pos"].astype(str).str.upper().isin(["TE"])].copy()
    ...
    def _norm_school(s):
        aliases = {"Penn St.": "Penn State", "Ohio St.": "Ohio State",
                   ...
                   "Kansas St.": "Kansas State", "Bowling Green": "Bowling Green"}
        x = str(s).strip() if pd.notna(s) else ""
        return aliases.get(x, x)

    dp_te = dp_te.rename(columns={"Rnd": "Round"})
    dp_te["Player_n"] = dp_te["Player"].map(_norm_name)
    dp_te["School_n"] = dp_te["School"].map(_norm_school)
    te_2025 = te_2025.drop(columns=["Round", "Pick"], errors="ignore")
    te_2025["Player_n"] = te_2025["Player"].map(_norm_name)
    te_2025["School_n"] = te_2025["School"].map(_norm_school)
    te_2025 = te_2025.merge(
        dp_te[["Player_n", "School_n", "Round", "Pick"]], on=["Player_n", "School_n"], how="left"
    ).drop(columns=["Player_n", "School_n"], errors="ignore")
    te_2025["Round"] = te_2025["Round"].fillna(8)
    te_2025 = te_2025[te_2025["Round"] < 8].copy()
    print(f"2025 TEs after draft-pick filter (drafted only): {len(te_2025)}")


# ─── Load 2026 TEs from JordanSportGuy combine file ──────────────────────────

combine_2026_path = os.path.join(
    DATA_RAW, "2026_Combine",
    "NFL Combine 2026 @JordanSportGuy Twitter - TEs.csv"
)
te_2026_list = []
if os.path.exists(combine_2026_path):
    ...
    for _, row in c26.iterrows():
        ...
        if not player or player.lower() in ("nan", ""):
            continue
        te_2026_list.append({
            "Year": 2026, "Player": player, "Pos": "TE", "School": school,
    ...
    print(f"Loaded 2026 TEs from JordanSportGuy: {len(te_2026_list)}")
else:
    print(f"2026 TE combine file not found at {combine_2026_path}")

te_2026 = pd.DataFrame(te_2026_list)


# ─── Assemble testing frame ───────────────────────────────────────────────────

testing_frames = [f for f in [te_2024, te_2025, te_2026] if not f.empty]
if testing_frames:
    te_testing_data = pd.concat(testing_frames, ignore_index=True)
else:
    te_testing_data = pd.DataFrame()
print(f"Total TE testing rows: {len(te_testing_data)}")


# ─── Merge PFF, RAS, arm length ───────────────────────────────────────────────

training_raw = add_pff_receiving(training_raw, receiving_data)
training_raw = add_pff_run_blocking(training_raw, run_block_data)
if not te_testing_data.empty:
    te_testing_data = add_pff_receiving(te_testing_data, receiving_data)
    te_testing_data = add_pff_run_blocking(te_testing_data, run_block_data)

RAS_PATH = os.path.join(DATA_RAW, "ras.csv")
training_raw = add_ras_data(training_raw, RAS_PATH)
if not te_testing_data.empty:
    te_testing_data = add_ras_data(te_testing_data, RAS_PATH)

arm_path = os.path.join(DATA_RAW, "mockdraftable_te_arm_length.csv")
if os.path.exists(arm_path):
    ...
    if {"Player", "Year", "arm_length_inches"}.issubset(arm_df.columns):
        ...
        print(f"Arm length TE: {len(arm_df)} records")
    else:
        arm_df = pd.DataFrame(columns=["Player", "Year", "arm_length_inches"])
else:
    arm_df = pd.DataFrame(columns=["Player", "Year", "arm_length_inches"])
    print("No mockdraftable_te_arm_length.csv; arm_length_inches from combine files only.")

training_raw = add_arm_length(training_raw, arm_df)
if not te_testing_data.empty:
    te_testing_data = add_arm_length(te_testing_data, arm_df)
...
if not te_testing_data.empty:
    te_testing_data = reorder_cols(te_testing_data, COL_ORDER)
...
pff_cols = ["player_game_count", "yprr", "yards_per_reception", "caught_percent",
            "avg_depth_of_target", "targeted_qb_rating", "contested_catch_rate",
            "drop_rate", "yards_after_catch_per_reception", "avoided_tackles",
            "slot_rate", "wide_rate", "inline_rate", "route_rate", "grades_run_block"]
for col in pff_cols:
    if col in training_raw.columns:
        n = training_raw[col].notna().sum()
        print(f"Training TEs with {col}: {n}/{len(training_raw)}")
...
training_raw.to_csv(os.path.join(DATA_PROCESSED, "te_training.csv"), index=False)
print(f"Saved te_training.csv: {len(training_raw)} rows (2015–2023)")

if not te_testing_data.empty:
    te_testing_data.to_csv(os.path.join(DATA_PROCESSED, "te_testing.csv"), index=False)
    print(f"Saved te_testing.csv: {len(te_testing_data)} rows (2024–2026)")
else:
    pd.DataFrame(columns=COL_ORDER).to_csv(
        os.path.join(DATA_PROCESSED, "te_testing.csv"), index=False
    )
    print("Saved te_testing.csv (empty)")
