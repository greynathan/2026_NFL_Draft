#!/usr/bin/env python3
"""
Scrape MockDraftable for arm length only for players in our te_training.csv
and te_testing.csv. Saves to data/raw/mockdraftable_te_arm_length.csv
for use in TE data_cleaning (merge by Player + Year).
"""
import re
import time
import csv
...
# Paths: script in TE/, data in project root
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)
DATA_DIR = os.path.join(PROJECT_ROOT, "data")
TRAINING_PATH = os.path.join(DATA_DIR, "processed", "te_training.csv")
TESTING_PATH = os.path.join(DATA_DIR, "processed", "te_testing.csv")
DRAFTED_2026_PATH = os.path.join(SCRIPT_DIR, "te_drafted_2026.csv")
OUTPUT_PATH = os.path.join(DATA_DIR, "raw", "mockdraftable_te_arm_length.csv")
...
def main():
    ...
    print(f"Loaded {len(players)} unique players from te_training + te_testing.")

    rows = []
    for i, (player_name, year, school) in enumerate(players):
        slug = name_to_slug(str(player_name))
        if not slug:
