#!/usr/bin/env python3
"""
Scrape MockDraftable for arm length only for players in our ot_training.csv,
ot_testing.csv, and ot_drafted_2026.csv. Saves to data/raw/mockdraftable_ot_arm_length.csv
for use in OT data_cleaning (merge by Player + Year).
"""
import re
import time
import csv
...
# Paths: script in OT/, data in project root
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)
DATA_DIR = os.path.join(PROJECT_ROOT, "data")
TRAINING_PATH = os.path.join(DATA_DIR, "processed", "ot_training.csv")
TESTING_PATH = os.path.join(DATA_DIR, "processed", "ot_testing.csv")
DRAFTED_2026_PATH = os.path.join(SCRIPT_DIR, "ot_drafted_2026.csv")
OUTPUT_PATH = os.path.join(DATA_DIR, "raw", "mockdraftable_ot_arm_length.csv")

# MockDraftable uses OT for offensive tackles
POSITION = "OT"
...
def main():
    ...
    print(f"Loaded {len(players)} unique players from ot_training + ot_testing + ot_drafted_2026.")
